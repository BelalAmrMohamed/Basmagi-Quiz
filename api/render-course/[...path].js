// =============================================================================
// api/render-course/[...path].js
//
// Serverless function (Node.js runtime, catch-all dynamic route) that
// handles all requests to /course/:name and /course/:name/:sub1/:sub2/...
// Mirrors render-quiz.js's structure (Supabase lookup, HTML template read +
// meta-tag injection, caching headers) but differs in one key way: course
// pages are NOT a separate HTML file — they render inside the existing SPA
...
//
// What it does:
//   1. Reads the course slug + any nested folder slugs from req.query.path
//      (populated automatically by Vercel because this file uses the
//      "[...path].js" catch-all convention — see the handler below).
...
//
// URL contract:
//   /course/NAME[/SUB1/SUB2/...]  →  (vercel.json rewrite)  →
//   /api/render-course/NAME[/SUB1/SUB2/...]  →  (Vercel catch-all route
//   match)  →  this file, with req.query.path = [NAME, SUB1, SUB2, ...]
...
export default async function handler(req, res) {
    ...
    // This file lives at api/render-course/[...path].js — Vercel's catch-all
    // dynamic-route convention. vercel.json rewrites the /course/... subtree
    // with "/course/:path*" → "/api/render-course/:path*". Being a real
    // catch-all route file (not a flat api/render-course.js) is what makes
    // Vercel invoke THIS function for any depth of path under
    // /api/render-course/ at all — a flat file is never invoked for a
    // multi-segment request, and Vercel falls through to the SPA catch-all
    // instead (this was the original 404 bug).
    //
    // req.query.path is normally how Vercel exposes a catch-all route's
    // matched segments — but when the request arrives via a vercel.json
    // rewrite (rather than a direct hit on /api/render-course/...) in
    // `vercel dev`, req.query.path has been observed to come back empty
    // even though the function IS being invoked at the right path (this was
    // the second bug: an empty-segments fallback below turned that into an
    // incorrect redirect to "/"). So req.url is parsed directly as the
    // primary source of truth, exactly as this handler did before becoming
    // a catch-all file — req.query.path is kept only as a secondary check
    // in case a future Vercel/runtime version populates it reliably and
    // req.url ever doesn't carry the segments (e.g. some edge/proxy setups).
    // req.url inside a Vercel function can reflect either the rewritten
    // internal path ("/api/render-course/Website-Demo") or, in some
    // vercel dev versions, the original browser path
    // ("/course/Website-Demo") — strip whichever fixed prefix is present so
    // this works regardless of which one this Vercel CLI version supplies.
    const urlPath = (req.url || "").split("?")[0];
    const segmentsFromUrl = urlPath
        .replace(/^\/api\/render-course\/?/, "")
        .replace(/^\/course\/?/, "")
        .split("/")
        .filter(Boolean);

    const segmentsFromQuery = Array.isArray(req.query.path)
        ? req.query.path
        : req.query.path
            ? [req.query.path]
            : [];

    const rawSegments = segmentsFromUrl.length > 0 ? segmentsFromUrl : segmentsFromQuery;
    const allSegments = rawSegments.map((seg) => {
        try {
