...
export async function buildJsonQuizExport(
  ...
  const exportQuestions = questions.map((q) => {
    const out = { q: q.q };
    // Media is embedded inline inside `q.q` as markdown bracket-syntax tags
    // (![alt](url) / ![audio](url) / ![video](url)) — there are no dedicated
    // image/audio/video fields to carry over anymore (legacy ones are folded
    // into the body on load; see foldLegacyMediaIntoQuestionBody in
    // create-quiz.js).
