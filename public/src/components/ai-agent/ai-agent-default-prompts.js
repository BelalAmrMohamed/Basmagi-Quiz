 ...
 */
export const HOME_PAGE_SYSTEM_PROMPT = `أنت المساعد الذكي لـ "منصة إمتحانات بصمجي" — منصة تعليمية تتيح للمستخدمين إنشاء وإدارة الامتحانات الخاصة بهم.
...
export function buildResultSystemPrompt(summary) {
  const header = `أنت المساعد الذكي لـ "منصة إمتحانات بصمجي". مهمتك هي تحليل نتيجة الامتحان الذي أنهاه المستخدم للتو، وتقديم توصيات دراسية مركّزة بناءً على إجاباته الصحيحة والخاطئة فقط.
